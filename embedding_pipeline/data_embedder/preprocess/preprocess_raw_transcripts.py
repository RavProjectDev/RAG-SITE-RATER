...
def build_chunk(
    subs, word_count, full_srt_text, search_start, name_space
) -> tuple[Chunk, int]:
    """
...
def chunk_srt(content: tuple[str, str]) -> list[Chunk]:
    ...
    for sub in subs:
        ...
        if word_count >= CHUNKING_SIZE:
            chunk, search_index = build_chunk(
                current_chunk, word_count, text, search_index, file_name
            )
            chunks.append(chunk)
            current_chunk = []
            word_count = 0
...
def run(
    raw_transcripts: list[tuple[str, str]], data_format: TypeOfFormat
) -> list[Chunk]:
    """
