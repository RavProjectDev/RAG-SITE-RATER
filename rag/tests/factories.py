...
def make_chunk(randomize=False, include_times=False, **overrides):
    data = {
        "text": random_string(50) if randomize else "default text",
...
def make_vector_embedding(randomize_vector=False,randomize_metadata = False, randomize_sanity_data=False, **overrides):
    vector_length = 748
    data = {
        "vector": [random.uniform(-1, 1) for _ in range(vector_length)] if randomize_vector else [0.1] * vector_length,
